...
# chal1 
print(sum([int(a<=c<=b and a<=d<=b or c<=a<=d and c<=b<=d)for (a,b),(c,d)in [[list(map(int,i.split('-')))for i in l.split(',')]for l in open('v').read().splitlines()]]))

# chal2
print(sum([int(a<=c<=b or a<=d<=b or c<=a<=d or c<=b<=d)for (a,b),(c,d)in [[list(map(int,i.split('-')))for i in l.split(',')]for l in open('v').read().splitlines()]]))
